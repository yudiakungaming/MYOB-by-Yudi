...
import { MyobCompany, MyobAccount, MyobJournal, MyobCard, InventoryItem } from './types';
...
export default function App() {
  ...
  const [inventoryDefaultTab, setInventoryDefaultTab] = useState<'list' | 'adjust'>('list');
  ...
  // Sync accounting databases (accounts, journals, cards, inventory) for selected active company file
  useEffect(() => {
    ...
    const inventoryPath = `users/${user.uid}/companies/${activeCompanyId}/inventoryItems`;
    ...
    // 4. Snapshot Inventory Items (Stok)
    const unsubInventory = onSnapshot(
      collection(db, 'users', user.uid, 'companies', activeCompanyId, 'inventoryItems'),
      (snapshot) => {
      ...
      (err) => {
        console.error('Error snapshot inventoryItems:', err);
        handleFirestoreError(err, OperationType.GET, inventoryPath);
      }
    );
  ...
  return (
    <div id="app_workspace" className="min-h-screen bg-stone-100 flex flex-col justify-between font-sans leading-relaxed text-stone-800">
      ...
      <main className="flex-1 p-6 flex items-center justify-center min-h-[500px]">
        {showSetupWizard ? (
          ...
          <CommandCentre 
            ...
            onOpenInventoryList={() => {
              setInventoryDefaultTab('list');
              setIsInventoryListOpen(true);
            }}
            onOpenAdjustInventory={() => {
              setInventoryDefaultTab('adjust');
              setIsInventoryListOpen(true);
            }}
            onOpenSalesInvoice={() => setIsSalesInvoiceOpen(true)}
            onOpenPurchaseBill={() => setIsPurchaseBillOpen(true)}
          />
        ) : (
      ...
      {/* 8. Inventory Items List & Adjustments Modal */}
      {isInventoryListOpen && activeCompanyId && (
        <InventoryListModal 
          userId={user.uid}
          companyId={activeCompanyId}
          accounts={accounts}
          ...
          defaultTab={inventoryDefaultTab}
        />
      )}

      {/* 9. Sales Invoice Modal */}
      {isSalesInvoiceOpen && activeCompanyId && (
        <SalesInvoiceModal 
          userId={user.uid}
          companyId={activeCompanyId}
          accounts={accounts}
      ...
      {/* 10. Purchase Bill Modal */}
      {isPurchaseBillOpen && activeCompanyId && (
        <PurchaseBillModal 
          userId={user.uid}
          companyId={activeCompanyId}
          accounts={accounts}
